...
import { getAllCaseStudies, getAllTestimonials, getComparison, getOurModelPage } from '@/lib/content';
...
export default async function OurModelPage() {
  const [caseStudies, allTestimonials, comparison, page] = await Promise.all([
    getAllCaseStudies(),
    getAllTestimonials(),
    getComparison(),
    getOurModelPage(),
  ...
  return (
    <>
      ...
      <IconGrid
        title={page.iconGridTitle}
        description={page.iconGridDescription}
        items={TEAM_ROLES}
        columns={8}
        size="default"
        id="team-model"
      ...
      <CaseStudyGrid
        ...
        background="muted"
        items={caseStudies.filter((s) => s.title !== 'Oats Overnight').map((s) => ({
          title: s.title,
